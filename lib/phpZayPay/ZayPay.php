...
class ZayPay
{
    ...
    public function __construct ($price_setting_id = null, $price_setting_key = null) {
        if ($price_setting_id !== null and is_numeric($price_setting_id)) {
            $this->price_setting_id = (int) $price_setting_id;
        }
     ...
     * @param string $locale
     * @param int $method_id
     * @param int $optional_amount
     * @param int $payalogue_id
     * @param array $your_variables
     * @return array with payment data
     */
    public function create_payment ($locale, $method_id, $optional_amount = null, $payalogue_id = null, $your_variables = null) {
        $url="/{$optional_amount}/{$locale}/pay/{$this->price_setting_id}/payments/create?key={$this->price_setting_key}&payment_method_id={$method_id}";
        if($payalogue_id)
            $url.="&payalogue_id={$payalogue_id}";
        if($your_variables)
            foreach($your_variables as $k => $v)
